...
class RecursiveCharacterChunker(BaseChunker):
    ...
    def chunk(self, text: str, metadata: Optional[dict] = None) -> List[TextChunk]:
            ...
            if len(candidate) <= self.chunk_size:
                ...
                if buffer:
                    ...
                    chunks.append(TextChunk(
                        content=buffer, chunk_index=len(chunks),
                        start_char=max(0, start), end_char=end,
        ...
        logger.info(f"Chunking complete: strategy=recursive, chunks={len(chunks)}")
        return chunks


class SemanticChunker(BaseChunker):
    def __init__(self, max_tokens: int = settings.chunk_size, overlap_sentences: int = 2):
    ...
    def chunk(self, text: str, metadata: Optional[dict] = None) -> List[TextChunk]:
        ...
        for sentence in sentences:
            ...
            if current_tokens + s_tokens > self.max_tokens and current_sentences:
                ...
                chunks.append(TextChunk(
                    ...
                    start_char=max(0, start), end_char=max(0, start) + len(content),
        ...
        logger.info(f"Chunking complete: strategy=semantic, chunks={len(chunks)}")
        return chunks


class SlidingWindowChunker(BaseChunker):
    def __init__(self, window_size: int = settings.chunk_size, step_size: Optional[int] = None):
    ...
    def chunk(self, text: str, metadata: Optional[dict] = None) -> List[TextChunk]:
        ...
        logger.info(f"Chunking complete: strategy=sliding_window, chunks={len(chunks)}")
        return chunks
...
class ChunkerFactory:
    ...
    def create(cls, strategy: str = "recursive", **kwargs) -> BaseChunker:
        if strategy not in cls._registry:
            raise ValueError(f"Unknown chunking strategy '{strategy}'. Choose from: {list(cls._registry)}")
